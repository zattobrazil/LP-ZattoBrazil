...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 bg-[#fcf9f4] flex items-center justify-between px-6 py-6 lg:px-12 xl:px-16 2xl:px-24 transition-shadow duration-300 ${scrolled ? 'shadow-md' : 'shadow-none'}`}>
      <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="cursor-pointer select-none shrink-0">
        <Image
      ...
      <nav className="ml-6 lg:ml-8 xl:ml-12 2xl:ml-16">
        <ul className="hidden md:flex flex-row items-center gap-3 lg:gap-6 xl:gap-10 2xl:gap-20">
          {navLinks.map((link) => {
            ...
            return (
              <li key={link.id} className={`transition-transform duration-300 ${isActive ? 'scale-110' : 'scale-100'}`}>
                <Link
                  href={`#${link.id}`}
                  className={`relative inline-block whitespace-nowrap font-sans font-light text-[10px] md:text-[11px] lg:text-xs xl:text-sm 2xl:text-lg uppercase tracking-wide text-[#213655] transition-opacity duration-300 cursor-pointer
